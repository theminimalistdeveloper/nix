{self, ...}:{
  programs.nixvim = {
    ...
    { key = "<leader>fc"; action = "<cmd>Telescope commands theme=ivy<cr>";}
    # Search through Neovim options
    { key = "<leader>fvo"; action = "<cmd>Telescope vim_options theme=ivy<cr>";}
    # Search through the loaded configuration files and reload it
    { key = "<leader>fl"; action = "<cmd>Telescope reloader theme=ivy<cr>";}
    # Search through the files in the current folder
    { key = "<leader>ff"; action = "<cmd>Telescope find_files theme=ivy<cr>";}
    # Searhc through the files known to Git in the current git folder
    { key = "<leader>fgf"; action = "<cmd>Telescope git_files theme=ivy<cr>";}
    # Search through the available Git branches
    { key = "<leader>fgb"; action = "<cmd>Telescope git_branches theme=ivy<cr>";}
    # Search through the available Git commits
    { key = "<leader>fgc"; action = "<cmd>Telescope git_commits theme=ivy<cr>";}
    # Search through the available buffers
    { key = "<leader>fb"; action = "<cmd>Telescope buffers theme=ivy<cr>";}
    # Search through the files in the current folder using grep syntax
    { key = "<leader>fgr"; action = "<cmd>Telescope live_grep theme=ivy<cr>";}
    ];
  };
}
